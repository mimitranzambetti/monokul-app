...
#   for channel in channels:
  ...
  if not dryRun:
    parentDir = os.path.join(os.pardir, 'intermediate_files', 'slack_messages', 'channels')
    mkdir(parentDir)
    for channel in channels:
        # print("getting history for channel {0}".format(channel['name']))
        fileName = os.path.join(parentDir, channel['name'] + '.json')
...
def getDirectMessages(slack, ownerId, userIdNameMap, dryRun):
  ...
  if not dryRun:
    parentDir = os.path.join(os.pardir, 'intermediate_files', 'slack_messages', 'direct_messages')
    mkdir(parentDir)
    for dm in dms:
        name = userIdNameMap.get(dm['user'], dm['user'] + " (name unknown)")
        print("getting history for direct messages with {0}".format(name))
        fileName = os.path.join(parentDir, name + '.json')
...
#   for group in groups:
  ...
  if not dryRun:
    parentDir = os.path.join(os.pardir, 'intermediate_files', 'slack_messages', 'private_channels')
    mkdir(parentDir)

    for group in groups:
        messages = []
        #   print("getting history for private channel {0} with id {1}".format(group['name'], group['id']))
        fileName = os.path.join(parentDir, group['name'] + '.json')
...
def find_slack_channels_and_users(token=None):
    if token is None:
        api_token = raw_input("Please enter your Slack Legacy Token:")
    else:
        api_token = token
    ...
    channels = slack.channels.list().body['channels']
    channels += groups
    ret_obj = {'users': userIdNameMap, 'channels': channels}
    return json.dumps(ret_obj)


def download_slack_history(token=None):
    if token is None:
        api_token = raw_input("Please enter your Slack Legacy Token:")
    else:
        api_token = token
